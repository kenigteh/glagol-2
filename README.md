...
Скачай готовый `.dmg` из [последнего релиза](https://github.com/kenigteh/glagol-2/releases/latest), открой его и перетащи **Glagol** в «Программы».

**При первом запуске** macOS не даст открыть приложение сразу — оно ещё без подписи Apple. Лечится за полминуты:

1. Дважды кликни по **Glagol** — появится предупреждение, закрой его («Отменить»).
2. Открой **Системные настройки → Конфиденциальность и безопасность**.
3. Прокрути вниз до раздела **«Безопасность»** — там будет строчка о заблокированном Glagol. Нажми **«Открыть всё равно»**.
4. Подтверди паролем или Touch ID — дальше Glagol открывается обычным двойным кликом.

> 💡 Подпись Apple (чтобы установка была в один клик, без этого шага) — одна из целей сбора на [Boosty](https://boosty.to/sakovskii/donate).

При первом запуске выбранная модель скачается автоматически (по умолчанию Qwen 1.7B, ~1.6 ГБ). Дальше выдай доступ к **микрофону** и **Accessibility** (нужно, чтобы вставлять распознанный текст в активное поле), назначь хоткей — и можно диктовать.

<div align="center">
<img src="assets/overlay.png" alt="Glagol overlay во время диктовки" width="320" />
<br/>
...
Glagol — бесплатный проект, без подписок. Если он тебе понравится — буду рад поддержке. С её помощью я смогу развивать проект дальше и создавать новые приложения.
