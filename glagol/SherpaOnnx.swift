...
class SherpaOnnxOfflineRecognizer {
  ...
  /// Запускает декодирование на уже наполненный stream. `decode(samples:sampleRate:)`
  /// вызывает этот метод изнутри своего тела, но в вендорном файле
  /// (k2-fsa/sherpa-onnx swift-api-examples) сам метод отсутствует — добавлен
  /// здесь как обвязка над C-API `SherpaOnnxDecodeOfflineStream`.
